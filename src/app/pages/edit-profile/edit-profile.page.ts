...
export class EditProfilePage implements OnInit {
  ...
  update() {
    if (this.profileForm.controls.FatherContact.status === "INVALID") {
      ...
      if (this.utils.isOnline()) {
        ...
        this.utils.presentLoading();
        // this.profileForm.value.birthTime = this.formatAMPM(this.profileForm.value.birthTime);
        
        this.profileForm.value.AboutMe = (this.profileForm.value.AboutMe != '') ? JSON.stringify(this.profileForm.value.AboutMe) : ''
        this.profileForm.value.Hobbies = (this.profileForm.value.Hobbies!= '') ? JSON.stringify(this.profileForm.value.Hobbies) : ''
        let data = {
          "id": UserId,
        ...
        this.httpService.httpPostwithHeader(this.httpService.Url.register, data, token).subscribe((res) => {
          ...
          if (this.parseData.message != undefined && this.parseData.message != '' && this.parseData.message!= null){
            this.utils.presentAlert(this.parseData.message);
          } else {
            this.utils.presentAlert('Profile Updated Successfully');
          }
  ...
  updatePage(profile) {
    console.log('profile ', profile)
     if(this.myProfile.AboutMe != '') 
    {
       this.myProfile.AboutMe =  this.myProfile.AboutMe.replace(/\\/g, '') 
       this.myProfile.AboutMe = this.myProfile.AboutMe.replaceAll('""', '');
      }
    if (profile.State) {
      this.getCity(profile.State);
    }

    if (this.myProfile.hobbies != "") {
      this.myProfile.hobbies = JSON.parse(profile.hobbies)
      profile.hobbies = this.myProfile.hobbies;
    }
  ...
  getDropDownData() {
    if (this.utils.isOnline) {
      ...
      this.httpService.httpGetwithHeader(this.httpService.Url.dropDownAPI, token).subscribe((res) => {
        ...
        this.parseData.hobbies_options = this.parseData.hobbies_options.filter(obj => {
          obj.selected = false
          return obj
        })
